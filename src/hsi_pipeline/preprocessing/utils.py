...
VARIANT_SUBDIRS = {
    "correcao": "correcao_rgb_bands",
    "correcao_snv": "correcao_snv_rgb_bands",
    "correcao_msc": "correcao_msc_rgb_bands",
...
def select_median_dir(variant_dirs: Dict[str, Optional[str]], preferred: str) -> Optional[str]:
    candidate = variant_dirs.get(preferred)
    if candidate:
        return candidate
    for key in VARIANT_ORDER:
        candidate = variant_dirs.get(key)
        if candidate:
            return candidate
    return None
