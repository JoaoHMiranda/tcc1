...
def _count_samples(datasets: Sequence[Path]) -> int:
    return sum(1 for dataset_dir in datasets for sample_dir in dataset_dir.iterdir() if sample_dir.is_dir())
...
def run_offline_augmentations(
    ...
    if progress and total_generated:
        progress.log(
            f"Augmentação offline: {total_generated} novas imagens ({per_sample} por amostra) em {samples_used} amostras.",
            style="yellow",
        )
    elif progress and not total_generated:
        progress.log("Augmentação offline habilitada, mas nenhuma imagem base/label foi encontrada.", style="yellow")
